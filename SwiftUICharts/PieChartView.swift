                ...
                GeometryReader { geometry in
                    ZStack  {
                        ForEach(0..<self.data.count){ i in
                            PieChartCell(center: CGPoint(x: geometry.frame(in: .local).midX, y: geometry.frame(in:  .local).midY),
...
struct PieChartView_Previews: PreviewProvider {
    static var previews: some View {
        PieChartView(title: "Chart Title",  data: ChartData.samplePieData, separatorColor: .black)
    }
}
